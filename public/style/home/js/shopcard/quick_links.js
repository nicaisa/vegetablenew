jQuery(function($){
	...
	//获取购物车数据
	function data(){
		var table = document.getElementById('quick_links_pop').querySelector('.shop-cart');
		var cells = document.getElementById('quick_links_pop').querySelectorAll('.cart_item');
		for (var i =  cells.length, len = i + 1; i < len; i++) {
			var li = document.createElement('li');
			var src='images/147_thumb_G_1409275114094.jpg';//图片路径
			var price='170.00';//单价
			var introduce ='夏季透气真皮豆豆鞋反绒男士休闲鞋韩版磨砂驾车鞋英伦船鞋男鞋子';//介绍
			var pnum=38;//斤数
			li.setAttribute('class','cart_item');
			li.innerHTML='<div class="cart_item_pic"><a href="#"><img src="'+src+'" /></a></div>\
								<div class="cart_item_desc">\
									<a href="#" class="cart_item_name">'+introduce+'</a>\
									<div class="cart_item_sku"><span>尺码：'+pnum+'码（精工限量版）</span></div>\
									<div class="cart_item_price"><span class="cart_price">'+price+'</span></div>\
								</div>';
			table.appendChild(li);
		}
	}
	var dataList=data();
	var totalPrice='170.00';//总价
	var num=1;//件数
	//具体数据操作 
	var 
	quickPopXHR,
			...
			content: '<div class="ibar_plugin_content">\
						<div class="ibar_cart_group ibar_cart_product">\
							<div class="ibar_cart_group_header">\
								<span class="ibar_cart_group_title">模板堂</span>\
								<a href="#">我的购物车</a>\
							</div>\
							<ul>\
							'+dataList+'\
					...
					<div class="cart_handler">\
						...
						<span class="cart_handler_left">共<span class="cart_price">'+num+'</span>件商品</span>\
						<span class="cart_handler_right">'+totalPrice+'</span>\
