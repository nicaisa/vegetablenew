	...
	<body class="forget-password authenticate">
				...
				<div class="maincenter">
					...
					<div class="maincenter-box" style="margin-top: 50px;">
						...
						<div class="ui-form-item">
							...
							<p class="ui-form-text"><span class="ui-text-amount">15736556255</span></p>
						</div>
						<div class="ui-form-item">
		...
		<script type="text/javascript">
			if(sessionStorage.getItem('phone')){
			    $(".ui-text-amount").html(sessionStorage.getItem('phone'));
			}
			...
			$("#J-submit-btn").on("click",function(){
				...
				var phone= $(".ui-text-amount").html();
				checkPwd(newval,newPwd);
				checkPwd(val,pwd);
				equal(val,newval,pwd);
				if(equal(val,newval,pwd)){
				    $.post("{{route('rs')}}",{"new_password":newval,'phone':phone,'_token': '{{csrf_token()}}'},function (reg) {
						if(reg.status==1){
                            window.location.href="{{route('rss')}}";
						} else if(reg.status==0){
                            window.location.href="{{route('forget')}}";
						}
                    },'json')
			...
			function checkPwd(val,obj){
				if(val == ""||val==null){
					...
					return checkPassword(obj,val,'必须是6-18位英文字母、数字或符号');
				}
			}
			//检查input是否为空
