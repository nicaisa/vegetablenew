	...
	<div class="container">
		<div class="row">
               <div class="col-md-4" style="padding:20px 0 0 0;">
                   ...
                   <div class="add-group">
                        <a href="{{route('sizeshow')}}"  class="btn btn-success">添加</a>
                   </div>
               </div>
        </div>
		<br/>
...
<script type="text/javascript">
   ...
   function status(obj,id,status){
          if (status) {
           ...
           //ajax提交参数都是根据id:id方式提交
             $.post('{{route("sizestatus")}}',{id:id,"_token":"{{csrf_token()}}","status":"0"},function(data){
               // 正常变为禁用
                  if (data.status==1) {
                  alert(data.message);  
               ...
               //禁用变为正常json一定要记住写
                 $.post('{{route("sizestatus")}}',{id:id,"_token":"{{csrf_token()}}","status":"1"},function(data){
                      if (data.status==1) {
                         //弹出状态码
                         alert(data.message);  
   ...
   function sou(page){
       ...
       $.get("{{route('size')}}", { 'pages':page,'name':name},function(data){
           //替换整个页面
           //$(".container").html(data);
       ...
       for(var i=0;i<data.length;i++){
           ...
           str+="<tr>\n" +
			   ...
			   "<td><span class=\""+classname+"\"onclick=\"status(this,"+data[i]["id"]+","+data[i]['status']+")\">"+status+"</span></td>" +
				   "<td><a   class=\"btn btn-danger btn-xs\" onclick='del(this)' data-id="+data[i]["id"]+">删除</a>\n" +
